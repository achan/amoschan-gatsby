  ...
  return (
    <div className="container mx-auto px-2 max-w-xl text-gray-700">
      ...
      <div>
        ...
        <ReactMarkdown className="mt-2 text-lg sm:text-xl" source={overview.body} />
      </div>
      <div className="mt-8">
        ...
        {experience.map((job, index) => {
          return (
            <div key={index} className="mb-10 last:mb-10">
              <h3 className="mt-2 font-bold text-xl font-serif">
                {job.company}, <span className="italic text-gray-600 text-md">{job.title}</span> 
              </h3>
              <div className="text-sm text-gray-500 tracking-wide">{job.tenure}</div>
              <Description>
                <ReactMarkdown className="mt-2" source={job.description} escapeHtml={false} />
              </Description>
              <div className="mt-2">
                {job.tags.map(t => <span className="bg-gray-400 text-white text-xs mr-2 px-2 py-1 rounded-full tracking-wide" key={t}>#{t}</span>)}
              </div>
            </div>
          )
        })}
